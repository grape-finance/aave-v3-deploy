 ...
 * @dev Favor Token
 */
contract MintFavorableERC20 is IERC20WithPermit, ERC20 {
    ...
    uint256 public sellTax = 50; // 50%
    uint256 public buyTax = 0; // 0%
    mapping(address => bool) public whitelist;

    constructor() ERC20("Favor Token", "FAVOR") {
    ...
    function setSellTax(uint256 _sellTax) external onlyOwner {
        require(_sellTax <= 50, "Tax cannot exceed 50%");
        sellTax = _sellTax;
    }

    function whitelistAddress(address addr, bool status) external onlyOwner {
    ...
    function _transfer(
        ...
        if (!whitelist[sender] && !whitelist[recipient]) {
            uint256 tax = (recipient == address(this))
                ? (amount * sellTax) / 100
                : 0;
